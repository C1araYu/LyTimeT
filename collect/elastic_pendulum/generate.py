...
def engine(rng, num_frm, fps=60):
    # physical parameters
    L_0 = 0.5    # elastic pendulum unstretched length (m)
    L = 0.5      # rigid pendulum rod length (m)
    m = 1.0      # rigid pendulum mass (kg)
    g = 9.81     # gravitational acceleration (m/s^2)
    k = 400.0     # elastic constant (kg/s^2)

    dt = 1.0 / fps
    t_eval = np.arange(num_frm) * dt
    
    # solve equations of motion
    # y = [theta_1, theta_2, z, omega_1, omega_2, vel_z]
    def f(t, y):
        La = L_0 + y[2]
        Lb = 0.5 * L
        I = (1. / 12.) * m * (L ** 2)
    ...
    rej = True
    while rej:
        initial_state = [rng.uniform(0, 2*np.pi), rng.uniform(0, 2*np.pi), rng.uniform(-0.04, 0.04), rng.uniform(-6, 6), rng.uniform(-10, 10), rng.uniform(-0.1, 0.1)]
...
def gen_data(data_filepath, num_seq, num_frm, seed=0):
    ...
    np.save('/root/data/physics_prediction_v2/data/elastic_pendulum/states.npy', states)

if __name__ == '__main__':
    data_filepath = '/root/data/physics_prediction_v2/data/elastic_pendulum'
